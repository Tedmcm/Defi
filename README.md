...
CoinDrive started as a vision to decentralize data storage and streaming and put the power to control/own your data in your hands rather any corporate or centralized entitiy. It was concieved and built by a team of dreamers at Columbia University's Fintech Bootcamp. Our mission was to create a basic system that would be adaptable to multiple use cases and business models. The basic structure was a system that allowes users to upload data (text, pictures or video) through our website to the IPFS system and it would simultaneously generate an ERC721 token holding the unique hash from the IPFS system inside the token, and this token would then be sent to your mycrypto wallet. For the project we built a proof of work blockchain, completed an ICO with an ERC20 token we designed with solidity on the Ropsten test net, coded and deployed various smart contracts to run the business once operational (revenue split, equity ownership split, user and content creator rewards system, mining rewards system, ERC20 and ERC721 tokens and an employee wallet). The following is a users guide to set up the system and get started. Updates will be coming.....
...
## Once the CoinDrive blockchain is ReActivated and Running we will be offering serveral different mining solutions. 
